...
def check(q, text, seen):
    ...
    # Порядок вариантов у каждого участника свой (страница экзамена перемешивает их), поэтому
    # «первая формулировка» или «вариант b» в пояснении или формулировке указывают не туда.
    # Нашлось на живом прогоне: разбор говорил «только первая формулировка», а верная стояла третьей.
    positional = re.compile(r'(перв|втор|трет|четв[её]рт|последн)[а-яё]*\s+(формулировк|вариант|ответ|пункт)'
                            r'|вариант[а-яё]*\s+[a-gа-г]\b|\b[a-g]\)', re.I)
    for field in ('explanation', 'stem'):
        if positional.search(q.get(field) or ''):
            problems.append('ссылка на порядок вариантов в поле «%s» — у участника порядок другой' % field)
    return problems
